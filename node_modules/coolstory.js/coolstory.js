// core generator
var coolstory = (function() {
    var dictionary = {};

    function addEntries(entries) {
        for (var entry in entries) {
            if (entries.hasOwnProperty(entry)) {
                dictionary[entry] = (dictionary[entry] || []).concat(entries[entry]);
            } 
        }
    }
    ...
    function fillStringMaxLength(template, maxLength) {
        for (var i = 0; i < 10000; ++i) {
            var str = fillString(template);
            if (str.length <= maxLength) return str;
        }
        throw new Error("Couldn't generate a short enough string");
    }

    function dictRep(match, inner) {
        var x = dictionary[inner];
        if (!x) throw new Error('Unknown template type: '+inner);
        return x[Math.floor(Math.random()*x.length)];
    }

    var coolstory = function(templateName, maxLength) {
        if (maxLength > 0) return fillStringMaxLength(':'+templateName, maxLength);
        else return fillString(':'+templateName);
    }

    coolstory.add = addEntries;
    coolstory.title = coolstory.bind(coolstory, 'Title');
    return coolstory;
})();
